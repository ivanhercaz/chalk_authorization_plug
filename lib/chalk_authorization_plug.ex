defmodule ChalkAuthorization.Plug do
  ...
  def require_authorization(%{assigns: %{current_user: %{__struct__: struct} = user}} = conn, [permission: {action, element}]),
    do: if(struct.can?(user, action, element), do: conn, else: unauthorize(conn))

  def require_authorization(%{assigns: %{current_user: %{__struct__: struct} = user}} = conn, [group: group]),
